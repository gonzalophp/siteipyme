...
.directive('displayproduct', function(){
    return {
        ...
        replace:true,
        template:'<table><tbody><tr><td rowspan="3"><a href="#/shop/product/{{product.id}}"><img src="{{product.image_path}}" alt="{{product.longDescription}}"/></a></td><td><span>{{product.description}}</span></td></tr><tr><td><span>{{product.currency}}</span>&nbsp;<span>{{product.price}}</span></td></tr><tr><td><button class="shop addtobasket" ng-click="addbutton(product)">Add</button><quantity class="basketquantity" ng-model="product.quantity"></quantity></td></tr></tbody></table>',
        link:function(scope, element, attr) {
        }
    }
})
